...
``python -m probecost.cli`` for the bare ``probecost`` entry point so the suite
...
def test_the_parser_found_the_steps():
    ...
    assert any("cross-instrument comparison still raises" in n for n in names)
...
def shim_path(tmp_path_factory):
    """A directory on PATH holding a ``probecost`` that runs this source tree.

    The first version of this test rewrote the workflow text with a regex
    instead, and ``\\bprobecost\\b`` promptly matched ``probecost.model`` inside
    ...
    for name, args in (("probecost", "-X utf8 -m probecost.cli"),
                       ("ruff", "-m ruff"),
                       ("python", ""), ("python3", "")):
        p = d / name
